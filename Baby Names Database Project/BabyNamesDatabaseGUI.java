...
public class BabyNamesDatabaseGUI extends JFrame implements ActionListener{
    ...
    public void setupGUI() {
        ...
        mostPopularButton = new JButton ("By Most Popular");
        topTenButton = new JButton ("By Top Ten");
    ...
    private void displayMostPopular (){
        try{
            ...
            BabyName result = database.mostPopularGirl(number);
            if (result == null)
                JOptionPane.showMessageDialog(this,"No Popular Girl Found for Given Year");
            else
            {
            ...
            result = database.mostPopularBoy(number);
            if (result == null)
                JOptionPane.showMessageDialog(this,"No Popular Boy Found for Given Year");
            else
            {
                resultsArea.append ("\n Most Popular Boy for year " + number);
                resultsArea.append ("\n" + result);
            }
    ...
    private void displayByYear () {
        try{
            int number = Integer.parseInt(yearTextField.getText());  
            ArrayList<BabyName> list =database.searchForYear (number);
            if (list.size() == 0)
                JOptionPane.showMessageDialog(this,"No Data found for given year");
            else
                displayNames (list);
        }      
        catch(NumberFormatException e){
    ...
    private void displayTopTen () {
        try{
            int number = Integer.parseInt(yearTextField.getText()); 
            ArrayList<BabyName> list = database.topTenNames(number);
            if (list.size() == 0)
                JOptionPane.showMessageDialog(this,"No data found for given year");
            else
            displayNames (list);
    ...
    private void displayByName () {
        try{
            ...
            {
                ArrayList<BabyName> list = database.searchForName(name);
                if (list.size() == 0)
                    JOptionPane.showMessageDialog(this,"No Data found with given Name");
                else
                    displayNames (list);
            }
        }      
    ...
    private void displayCounts () {
        DecimalFormat fmt = new DecimalFormat ("###,###");
        resultsArea.setText("");
        resultsArea.append ("Total: " +database.countAllNames() +"\n");
        int numOfGirls = database.countAllGirls();
        int numOfBoys = database.countAllBoys();
        if (numOfGirls == 0)
            JOptionPane.showMessageDialog(this,"No Girls Found in list");
        if (numOfBoys == 0)
            JOptionPane.showMessageDialog(this,"No Boys Found in list");
        resultsArea.append ("Total number of Girls: " +numOfGirls +"\n");
        resultsArea.append ("Total number of Boys: " +numOfBoys);
